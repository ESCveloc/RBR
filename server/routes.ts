...
export function registerRoutes(app: Express): Server {
  ...
  // Add the update location endpoint implementation
  app.post("/api/games/:gameId/update-location", async (req, res) => {
    if (!req.isAuthenticated()) {
      return res.status(401).send("Not logged in");
    }
    ...
    try {
      const gameId = parseInt(req.params.gameId);
      const { teamId, position, force = false } = req.body;
      ...
      // Allow both team captain and admin to update location
      if (participant.team.captainId !== req.user.id && req.user.role !== 'admin' && !force) {
        return res.status(403).send("Only team captain or admin can update location");
      }
